      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {['Dashboard', 'Events', 'Connect', 'About'].map((item) => (
              <a
                key={item}
              ...
              <a onClick={(e) => {
                e.preventDefault();
                isAuthenticated ? navigate('/dashboard') : navigate('/join');
              }}></a>
          ...
          <button className="md:hidden p-2 hover:bg-cyan-400/10 rounded-sm transition-colors duration-200">
            <Menu size={24} />
          </button>
        </div>
      </div>
